...
class JournalParser(private val api: EliteDangerousAPI) {
    ...
    private fun processJson(json: String, printJson: Boolean): String {
        var eventName = ""
        try {
            val defaultEvent = jacksonObjectMapper().readValue(json, DefaultEvent::class.java)
